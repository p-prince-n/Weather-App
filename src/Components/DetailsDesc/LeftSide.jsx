...
const LeftSide = () => {
  ...
  return (
    <div className="px-5 md:px-10 py-5 xl:px-15 flex items-center border rounded-xl shadow-2xl bg-black/20 border-gray-900   flex-col justify-center  ">
      ...
      <div className="flex items-center gap-5 md:gap-15  justify-center">
        <div className=" flex gap-2 items-center">
          ...
          <div className="flex flex-col gap-0.1">
            ...
            <div>{currentWeather?.main?.temp_min}°𝐶</div>
          </div>
        </div>
        ...
        <div className=" flex gap-2 items-center">
          ...
          <div className="flex flex-col gap-0.1">
            ...
            <div>{currentWeather?.main?.temp_max}°𝐶</div>
          </div>
        </div>
      </div>
      <div className="flex gap-1 my-2 text-sm md:text-sm">
        <div>feels like : </div>
        <p>{currentWeather?.main?.feels_like}°𝐶</p>
      </div>
      <div className="flex  items-center justify-center  gap-10">
        <div className="flex items-center justify-center gap-2 ">
          ...
          <div className="flex flex-col text-xs md:text-sm">
            <p>Sunrise</p>
            <p>{new Date(currentWeather?.sunrise * 1000).toLocaleTimeString()}</p>
          </div>
        </div>
        <div className="flex items-center justify-center gap-2 ">
          <Sunset size={25} />
          <div className="flex flex-col text-xs md:text-sm">
            <p>Sunset</p>
            <p>{new Date(currentWeather?.sunset * 1000).toLocaleTimeString()}</p>
          </div>
        </div>
      </div>
    </div>
